...
$.getJSON(link_to_data)
    .done(function(dictionary) {
        ...
        //console.log("testttTTT",dictionary[0].coordinates)
        //console.log("input3",$("#startingDate").getText());
        var input3 = document.getElementById("startingDate");
        var filter2 = input3.value.toUpperCase();
        var check ;
        if((document.getElementById("endingDate").value) == ""){check=true;}
        else{check=false;}
        console.log("input5",document.getElementById("endingDate").value,check);
        ...
        $(document).ready(function() {
            ...
            //  $("#startingDate").keyup(function() {
            //     is_valid_date();
            // });
            //  $("#endingDate").keyup(function() {
            //     is_valid_date();
            // });
 ...
 var sliderControl = L.control.sliderControl({
            position: "topright",
            layer: [blueLayer, redLayer, greenLayer, orangeLayer, violetLayer, greyLayer,manualLayer],
...
function processData(dictionary,) {
    ...
    for (var key in dictionary) {
        ...
        //
       plot_marker(dictValues,null);//[25.296637, 51.517686]);        
    ...
    console.log(personMarker);
        personMarker.on('dragend', function(event) {
         ...
         //   alert(person_position);
            ...
            {
                 var dictValues = dictionary[key];
                 plot_marker(dictValues,person_position);
...
function to_12H(time24){
    ...
    if(hour>=12){time12=time12+" PM";}
    else{time12=time12+" AM";}
    return time12
...
function compare_time(t1,t2)
{
    ...
    //compare AM/PM
    if(offTime[2]=="AM" && offTime2[2]=="PM"){return 2;}
    if(offTime2[2]=="AM" && offTime[2]=="PM"){return 1;}
    
    //compare hours
    if(offTime[0]>offTime2[0]){return 1;}
    if(offTime2[0]>offTime[0]){return 2;}
    //compare minutes
    if(offTime[1]>offTime2[1]){return 1;}
    if(offTime2[1]>offTime[1]){return 2;}
    return 0;
}
...
function is_valid_date(eventTiming,userStartTiming,userEndTiming)//ex: "2017-06-07 00:00:00 2017-05-21 00:00:00",06/14/2017 12:00 AM,06/28/2017 12:00 AM
{
   ...
   // var eventTiming="  2017-06-07 10:00:00 2017-05-21 17:00:00";
    ...
    if(moment(eventEndDate,'YYYY-MM-DD').isBefore(eventStartDate,'YYYY-MM-DD')){return false;}
    if(moment(eventEndDate,'YYYY-MM-DD').isSame(eventStartDate,'YYYY-MM-DD') &&
                 (compare_time(eventStartTime,eventEndTime)==1)){return false;}
    if(userStartTiming ==null && userEndTiming == null)
    {
        if(moment(today,'YYYY/MM/DD').isAfter(eventEndDate,'YYYY-MM-DD')){return false} 
        if(moment(today,'YYYY/MM/DD').isSame(eventEndDate,'YYYY-MM-DD') && compare_time(eventEndTime,currentTime)==1){return false;}
        return true;
    }
    //if(userStartTiming ==null){userStartTiming=}

    var userStartTimingSplit = userStartTiming.split(" ");
    //console.log("userStartTimingSplit",Date(userStartTimingSplit[0]));
    var userStartDate=userStartTimingSplit[0];
    var userStartTime = userStartTimingSplit[0]+" "+userStartTimingSplit[1];

    var userEndTimingSplit = userEndTiming.split(" ");
    var userEndDate=userEndTimingSplit[0];
    var userEndTime = userEndTimingSplit[0]+" "+userEndTimingSplit[1];   
    ...
    console.log("userEndTime",moment(eventEndDate,'YYYY-MM-DD')<moment(eventStartDate,'YYYY-MM-DD'));    

    if((moment(userEndDate,'MM/DD/YYYY').isBefore(userStartDate,'MM/DD/YYYY'))){alert("Not a valid time span");}

    if(moment(userEndDate,'MM/DD/YYYY').isSame(userStartDate,'MM/DD/YYYY')&&
            compare_time(userStartTime,userEndTime)==1){alert("Not a valid time span");}

    if(moment(userStartDate,'MM/DD/YYYY').isSame(eventStartDate,'YYYY-MM-DD') && (compare_time(eventEndTime,userStartTime)==2)){return false;}
    if(moment(userEndDate,'MM/DD/YYYY').isSame(eventEndDate,'YYYY-MM-DD') && (compare_time(userEndTime,eventStartTime)==2)){return false;}
...
function plot_valid_date(dictionary,userStartTiming,userEndTiming)
{
    ...
    for (var key in dictionary) {
        ...
        if(dateValidity){plot_marker(dictValues,null,dateValidity);}
    }
}
...
function plot_marker(eventDict,person_position=null,dateFilter=true,userStartTiming=null,userEndTiming=null) {
    ...
    if(eventStartTime=="00:00:00" ){eventStartTime=null;}
    if(eventEndTime=="00:00:00" ){eventEndTime=null;}
    ...
    if(is_valid_date(eventDict.date,userStartTiming,userEndTiming)){ marker.addTo(mapLayer); mapArr.push(marker);}
    else{return;}
    if(person_position!=null)
    {
        mapLayer.removeLayer(marker);
        if(get_distance(marker,person_position)<1000){
    ...
    //         if(get_distance(marker,person_position)<10000000){
    //         marker.addTo(mapLayer);
    //          }
    //         mapArr.push(marker);
    //     }
    //     else{
    //         if(get_distance(marker,person_position)<10000000 && is_valid_date(eventDict.date,null,null)){
    //         marker.addTo(mapLayer);
    //         }
    //         mapArr.push(marker);
    //     }
    ...
    // console.log("distanceTo", marker.getLatLng().distanceTo([24,55]));
...
function zoom_to_pin(dictionary, CatArr, Event) {
    ...
    var EventIndx = CatArr.indexOf(dictValues.category);
    console.log("index", CatArr,EventIndx,dictValues.category);
    //
    var marker = L.marker([dictValues.latitude, dictValues.longitude], {
            icon: iconArr[EventIndx]}).bindPopup(string)
